# File plan — `packs/connectors/spec-041/google-workspace.json`

**Status:** planned; no product acceptance implied.

Target: `packs/connectors/spec-041/google-workspace.json`. Representation: **sidecar-only**. Allocation: **required**.
...
```text
PROCEDURE ZN_0237 /* planning label, not a public API */
  OWNER := SPEC-041; TARGET := packs/connectors/spec-041/google-workspace.json
  REQUIRE accepted dependencies: ZN-0094, ZN-0111, ZN-0178, ZN-0191, ZN-0223
  ...
  TICKET-SPECIFIC SEGMENT:
    01. Declare Drive/document evidence scope, change cursor and deleted/permission-change handling using the admitted API profile.
    02. Map source principals and authorized content extracts to standard source bindings.
    03. Prove incremental resumption, ACL revocation and document revision provenance.
  TEST BEFORE DECLARING THIS SEGMENT COMPLETE:
    GIVEN A document revision changes and its sharing permission is removed between sync pages
    WHEN The recipe synchronizes
    THEN Revision lineage is retained and removed access prevents subsequent retrieval; source changes cannot be lost across checkpoint retries
