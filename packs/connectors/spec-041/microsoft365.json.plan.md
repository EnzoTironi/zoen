# File plan — `packs/connectors/spec-041/microsoft365.json`

**Status:** planned; no product acceptance implied.

Target: `packs/connectors/spec-041/microsoft365.json`. Representation: **sidecar-only**. Allocation: **required**.
...
```text
PROCEDURE ZN_0238 /* planning label, not a public API */
  OWNER := SPEC-041; TARGET := packs/connectors/spec-041/microsoft365.json
  REQUIRE accepted dependencies: ZN-0237
  ...
  TICKET-SPECIFIC SEGMENT:
    01. Declare the admitted Graph/SharePoint resource and delta-query profile.
    02. Map tenant identity, permission inheritance and source tombstones explicitly.
    03. Quarantine unsupported permission shapes instead of treating tenant access as unrestricted.
  TEST BEFORE DECLARING THIS SEGMENT COMPLETE:
    GIVEN A SharePoint item loses inherited access during delta synchronization
    WHEN The recipe updates source state
    THEN The stale item becomes undisclosable and no broad application token is interpreted as end-user permission
