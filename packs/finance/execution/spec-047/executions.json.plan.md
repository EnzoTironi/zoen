# File plan — `packs/finance/execution/spec-047/executions.json`

**Status:** planned; no product acceptance implied.

Target: `packs/finance/execution/spec-047/executions.json`. Representation: **sidecar-only**. Allocation: **required**.
...
```text
PROCEDURE ZN_0274 /* planning label, not a public API */
  OWNER := SPEC-047; TARGET := packs/finance/execution/spec-047/executions.json
  REQUIRE accepted dependencies: ZN-0273
  ...
  TICKET-SPECIFIC SEGMENT:
    01. Deduplicate scoped execution IDs and retain correction/bust chains.
    02. Compute active fill, leaves and reserved resources under exact decimal rules.
    03. Reject impossible overfills/corrections into reconciliation rather than silently clamp quantities.
  TEST BEFORE DECLARING THIS SEGMENT COMPLETE:
    GIVEN A 100-unit order fills 40 and then 10 of that execution is busted
    WHEN The events are admitted
    THEN Active fill becomes 30 and remaining quantity 70 before other events; both original and bust evidence remain visible
