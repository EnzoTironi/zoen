...
const csv = (separator: "\n" | "\r\n", quoteZero = false) =>
  `${header}${separator}d01.csv.v1,manual,billing,1,${quotedLabel},r1,invoice-1,obligation.amount,Known,${quoteZero ? '"0.00"' : "0.00"},BRL,Unknown,,${separator}`;
      ...
      withStorage(() =>
        Effect.gen(function* replayRepresentation() {
          ...
          for (const firstInput of representations.slice(0, 2)) {
            ...
            for (const input of [
              { document: csv("\n") },
              { document: json, format: "d01.csv.v1" },
              { document: csv("\n"), format: "d01.json.v1" },
            ]) {
              expect(
                yield* executor
