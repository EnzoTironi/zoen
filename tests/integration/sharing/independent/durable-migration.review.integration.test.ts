...
const oldSnapshot = Effect.gen(function* oldSnapshot() {
  ...
  const tables = yield* sql<{
    schema: string;
    name: string;
  }>`SELECT n.nspname AS schema, c.relname AS name FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE n.nspname IN ('authority', 'identity', 'jobs') AND c.relkind = 'r' AND c.relname NOT LIKE 'disclosure_%' AND c.relname NOT LIKE 'world_erasure_%' AND c.relname NOT LIKE 'object_write_%' AND c.relname <> 'action_log' ORDER BY 1, 2`;
  ...
  const tableRights =
    yield* sql`SELECT n.nspname, c.relname, c.relowner, c.relacl FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE n.nspname IN ('authority', 'identity', 'jobs') AND c.relkind = 'r' AND c.relname NOT LIKE 'disclosure_%' AND c.relname NOT LIKE 'world_erasure_%' AND c.relname NOT LIKE 'object_write_%' AND c.relname <> 'action_log' ORDER BY 1, 2`;
  const columnRights =
    yield* sql`SELECT n.nspname, c.relname, a.attname, a.attacl FROM pg_attribute a JOIN pg_class c ON c.oid = a.attrelid JOIN pg_namespace n ON n.oid = c.relnamespace WHERE n.nspname IN ('authority', 'identity', 'jobs') AND c.relkind = 'r' AND c.relname NOT LIKE 'disclosure_%' AND c.relname NOT LIKE 'world_erasure_%' AND c.relname NOT LIKE 'object_write_%' AND c.relname <> 'action_log' AND a.attnum > 0 AND NOT a.attisdropped ORDER BY 1, 2, 3`;
  const schemaRights =
