...
for (const action of ["revoke", "logout"] as const) {
  for (const stage of ["before-shared", "before-end"] as const) {
    it.live(
      `independent SH07/08 two real HTTP processes: ${stage} orders public ${action} and private emission`,
      () =>
        withD01Database(
          (database) =>
            withStorage((storage) =>
              Effect.scoped(
                Effect.gen(function* processOrdering() {
                  ...
                  if (stage === "before-shared") {
                    ...
                    expect(denied.status).toBe(action === "revoke" ? 404 : 401);
                    expect(denied.body).toStrictEqual(
                      action === "revoke"
                        ...
                        : { _tag: "Unauthenticated", code: "PRESENCE_REQUIRED" }
                    );
                  }
                })
              ).pipe(
