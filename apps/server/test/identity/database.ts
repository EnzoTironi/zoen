...
import type { IdentityConfig } from "../../src/identity/configuration.ts";
import { grantIdentityRole } from "../../src/identity/grants.ts";
import { makeIdentityLayer } from "../../src/identity/identity.ts";
  ...
  withWorldsDatabase((database) =>
    Effect.gen(function* prepareIdentity() {
      ...
      const source = yield* FileSystem.FileSystem.use((fs) =>
        fs.readFileString(
          fileURLToPath(
            new URL("../../src/identity/schema.sql", import.meta.url)
          )
        )
