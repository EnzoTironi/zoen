...
Contraprova adicional reproduziu escalada por `ADMIN OPTION` mesmo com `SET FALSE`: a role conseguiu conceder `SET TRUE` a si própria. A admissão recusa também qualquer `ADMIN OPTION` detida pela role atual ou por uma role alcançável via SET. A terceira regressão de memberships demonstra esse caminho antes do DDL real com rollback. São treze testes autorais EX06 após essa correção; revisões independentes são contadas separadamente.
