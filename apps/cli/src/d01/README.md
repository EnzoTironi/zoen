...
The HTTP transport uses `HttpApiClient.make(D01Api)` and the real EX09 identity endpoints. There are no authority, SQL, or storage imports. Four local tests exercise result/error formatting, URL validation, raw input boundaries, and session permissions/origin binding. They do not prove server behavior.
