...
import type { ImportDocument } from "@zoen/contracts/worlds/evidence";
import {
  EvidenceImported,
  ImportEvidence,
...
import type { ReceiptRef } from "@zoen/contracts/worlds/values";
...
import { EvidenceState } from "../ports/worlds/persistence.js";
...
export const importEvidence = Effect.fn("authority.evidence.importEvidence")(
  function* importEvidence(
    ...
    const reservation = yield* reserveCapture(
      ...
      request.worldRef,
      bytes,
      "format" in request.input ? request.input.format : "worlds.json.v1"
    );
