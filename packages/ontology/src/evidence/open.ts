...
import { CaptureState, EvidenceState } from "../ports/worlds/persistence.js";
import {
  CaptureId,
  EvidenceObjectStore,
  ObjectLocation,
...
const readLocation = Effect.fn("authority.evidence.readLocation")(
  function* readLocation(
    ...
    if (
      ...
      (location.documentFormat ?? "worlds.json.v1") !== evidence.document_format
    ) {
...
export const openEvidence = Effect.fn("authority.evidence.openEvidence")(
  function* openEvidence(
    ...
    return yield* Schema.decodeEffect(EvidenceOpened)({
      ...
      mediaType:
        documentFormat === "worlds.csv.v1" ? "text/csv" : "application/json",
