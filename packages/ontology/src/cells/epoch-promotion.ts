// @zoen-plan packages/ontology/src/cells/epoch-promotion.ts
// NON-EXECUTABLE PSEUDOCODE; not registered or compiled as product implementation.
// # File plan — `packages/ontology/src/cells/epoch-promotion.ts`
//
// **Status:** planned; no product acceptance implied.
//
// Target: `packages/ontology/src/cells/epoch-promotion.ts`. Representation: **comment-only-source**. Allocation: **required**.
//
...
//
// ```text
// PROCEDURE ZN_0248 /* planning label, not a public API */
//   OWNER := SPEC-042; TARGET := packages/ontology/src/cells/epoch-promotion.ts
//   REQUIRE accepted dependencies: ZN-0247
...
//     01. CAS the coordinator record using exact source fence and destination proof.
//     02. Install new epoch before enabling destination writes/routing.
//     03. Reject old request/lease fences and reconcile transferred effect IDs before any new send.
//   TEST BEFORE DECLARING THIS SEGMENT COMPLETE:
//     GIVEN Two destination candidates race promotion from the same source epoch
//     WHEN Both request activation
//     THEN Only one wins; all old-epoch writes/permits fail and no global superuser bypass exists
