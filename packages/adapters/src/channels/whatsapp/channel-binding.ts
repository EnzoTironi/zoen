// @zoen-plan packages/adapters/src/channels/whatsapp/channel-binding.ts
// NON-EXECUTABLE PSEUDOCODE; not registered or compiled as product implementation.
// # File plan — `packages/adapters/src/channels/whatsapp/channel-binding.ts`
//
// **Status:** planned; no product acceptance implied.
//
// Target: `packages/adapters/src/channels/whatsapp/channel-binding.ts`. Representation: **comment-only-source**. Allocation: **required**.
//
...
//
// ```text
// PROCEDURE ZN_0065 /* planning label, not a public API */
//   OWNER := SPEC-011; TARGET := packages/adapters/src/channels/whatsapp/channel-binding.ts
//   REQUIRE accepted dependencies: ZN-0064
...
//     01. Issue a short-lived challenge through an authenticated web session.
//     02. Bind the provider subject only after challenge verification and record consent separately.
//     03. Reject number/account reuse without renewed proof.
//   TEST BEFORE DECLARING THIS SEGMENT COMPLETE:
//     GIVEN A message comes from a known number but no current authenticated binding exists
//     WHEN It requests confidential company data
//     THEN Only a secure continuation/linking flow is offered; phone possession does not create membership or revive an old grant
