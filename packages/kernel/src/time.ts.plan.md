# File plan — `packages/kernel/src/time.ts`

**Status:** implementation-in-progress; no product acceptance implied.

Target: `packages/kernel/src/time.ts`. Representation: **existing-with-sidecar**. Allocation: **required**.

Specs: [SPEC-001](../../../docs/specs/spec-001.md).
Tickets: [ZN-0009](../../../docs/tickets/zn-0009.md).
...
```text
PROCEDURE ZN_0009 /* planning label, not a public API */
  OWNER := SPEC-001; TARGET := packages/kernel/src/time.ts
  REQUIRE accepted dependencies: ZN-0008
  ...
  TICKET-SPECIFIC SEGMENT:
    01. Define UTC Instant, LocalDate, zoned wall-time appointment and [from,until) intervals.
    02. Use an injected ClockSample for decisions; reject nonexistent or ambiguous wall time without explicit offset choice.
    03. Preserve valid time separately from recorded commit cut.
  TEST BEFORE DECLARING THIS SEGMENT COMPLETE:
    GIVEN Adjacent intervals ending and beginning at the same instant and a daylight-saving overlap fixture
    WHEN The temporal planner intersects intervals and parses the appointment
    THEN Adjacent half-open intervals do not overlap; ambiguous local time requires a choice; no host timezone changes the result
