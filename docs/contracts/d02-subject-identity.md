...
Um Frame de recuperação que cabe não prova que a Question de inversão cabe: efeitos e estado prospectivo também ocupam espaço. Todo writer de identidade deve validar prospectivamente a projeção estrutural e a Question completa necessárias para desfazer a própria decisão imediatamente, pelo mesmo serializador, antes de confirmar uma mudança. Metadados e impacto desse caminho são estruturais; não crescem com o inventário de claims. Mudanças posteriores de grafo continuam sujeitas aos impedimentos explícitos de undo já descritos. A prova deve demonstrar que aumentar somente claims não altera a capacidade de inspecionar e confirmar a inversão, incluindo bytes, entradas e profundidade; nenhuma amostra de fontes ou remoção de campo funcional vale como ajuste.
