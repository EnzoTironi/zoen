...
As escolhas deste candidato são: seletor CSV explícito no envelope; versão e metadados de uma única fonte presentes no próprio CSV; colunas e ordem fixas; nenhuma detecção de dialeto/localidade; mesma projeção `ImportDocument`; retenção do CSV original e formato de storage vinculado à captura. Root deve congelar essas escolhas, o handoff físico e os oráculos antes de despachar código.
...
6. Excesso de bytes do documento/envelope segue `QuotaExceeded` no executor; violações de gramática, campos, versão, cardinalidade de registros ou schemas seguem `InvalidInput`, consistente com a validação de registros JSON atual. Nenhuma mensagem pública inclui conteúdo, fragmento de célula, caminho de storage ou diagnóstico livre. Recusas locais do leitor web/CLI conservam os códigos locais existentes; não são prova de que o servidor validou o arquivo.
...
- `CaptureReservation` passa a carregar `documentFormat`. `EvidenceObjectStore.stage/locate` recebem esse formato; só a borda admitida o escolhe. `ObjectLocation` admite `documentFormat` opcional com esses literais. Localização histórica sem esse campo significa exclusivamente JSON legado; não inserir o campo retroativamente no JSON armazenado. Novas localizações registram o formato explícito.
