...
| [inspect.ts](../../packages/ontology/src/knowledge/inspect.ts) | Lê claims do World, grava Frame/pins e faz recheck após o snapshot. Frame histórico exige o mesmo principal, propósito e sujeito. Leitura semântica pode gravar metadados internos; não classificar permissões por verbos SQL. |
...
| [open.ts](../../packages/ontology/src/evidence/open.ts) | Lê bytes exatos de S3, verifica digest e relê localização/autorização antes de retornar. Não entrega chave, versão S3 ou URL presigned. |
...
| [grants.ts](../../apps/server/sql/proposals/worlds/grants.ts) e [identity/grants.ts](../../apps/server/src/identity/grants.ts) | Os papéis SQL são de componentes. Authority não lê identity; identity não escreve authority/jobs. Viewer é papel semântico de membership, não um login SQL novo. |
