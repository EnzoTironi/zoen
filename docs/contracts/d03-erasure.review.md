...
Inverter simplesmente as chamadas cria outro contraexemplo: validar owner/sessão/revisão, pausar antes do append, perder a autorização atual ou tornar a revisão confirmada obsoleta, e então concluir o append irrevogável com a decisão antiga. O commit local posterior deve rejeitar, mas o testemunho já pode suprimir o World. Um ledger negativo não pode transformar esse append prévio em autorização nova. É preciso um ponto externo irrevogável protegido pela decisão e pelo fencing admitidos, não duas observações de autorização em torno da rede. Incluir também perda de conexão do coordenador enquanto o append anterior ainda pode terminar.

Separar Closing interno de Suppressed público pode esclarecer a UX; sozinho isso não resolve uma intenção Unknown perdida no rollback quando a promessa é mais forte que a observação local. Um sinal externo de ausência tampouco prova que não existiu commit local perdido ou append em voo. A recuperação precisa da identidade estável da tentativa e de um protocolo que ordene origem, testemunho e ativação do restore.
