...
| --- | --- |
| Postgres loopback | SCRAM-SHA-256 only (no `trust`). Infra role `zoen_infra` password lives under `/data/zoen/bootstrap/` (root-only). |
...
| Fail-closed | Missing/rotated bootstrap admin password prevents readiness; app config rejects inherited `ZOEN_BOOTSTRAP_ADMIN_URL` / `ZOEN_S3_ADMIN_*`. |
