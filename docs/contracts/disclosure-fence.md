...
## Congelamento de implementação EX20–EX23 — 2026-09-05

Root adota o handoff revisado em [disclosure-handoff-review.md](disclosure-handoff-review.md). Esta seção substitui as alternativas ainda abertas acima, sem representar prova ou ativação. O limite público permanece **inclusivo em 1.048.576 bytes**. O gate dura pelo menos até o retorno de `ServerResponse.end`; o ramo no limite pode conservá-lo por mais tempo. Expiração e deadline são conferidos na última revalidação; nenhuma promessa de duração absoluta de transmissão ou cleanup foi aceita.

O executor mantém o DTO privado, valida o schema de sucesso da família e produz os bytes JSON antes de adquirir o gate. Um método de emissão por família recebe callback confiável `emit(Uint8Array)`, invocado somente depois da aquisição e revalidação final. O efeito exige o Scope do chamador. HTTP retorna `HttpServerResponse.uint8Array` por `handleRaw`, preservando o Scope real até o writer terminar. O contexto capturado na Layer omite `Scope.Scope`; dependências internas não sobrescrevem serviços arbitrários do callback. Métodos antigos de retorno de DTO permanecem compatibilidade em processo, com reautorização antes de produzir o resultado; não são API de emissão e nenhum adapter de rede usa esse caminho.

Os ports reais atribuídos por root são `ports/disclosure/fence.ts` e `ports/sharing/directory.ts`. `DisclosureFence.shared(presence, world, deadline)` registra no Scope os locks de sessão e membership, nessa ordem. `exclusiveSession(presence, deadline)` registra o exclusivo da sessão no Scope do logout. O port devolve apenas a duração do recurso, não uma conexão ou decisão de autorização. `checkHealth` verifica o coordenador real. `PrincipalDirectory.exists(principalId)` devolve somente booleano de existência pelo pool identity; o core autoriza o owner antes da consulta e dispensa consulta em replay já registrado.

As chaves são produzidas exclusivamente por `ports/disclosure/keys.ts`: prefixo versionado separado para sessão e membership, seguido do array JSON exato de realm/principal/session ou realm/world/principal. Leitores e escritores executam `hashtextextended(key, 0)` no **mesmo PostgreSQL físico de autoridade**. Uma colisão do hash apenas serializa recursos adicionais. A transação de grant/revoke adquire `pg_advisory_xact_lock` da chave do alvo antes de alterar sua membership e o conserva até COMMIT/ROLLBACK. O domínio membership e os locks de operação existentes continuam responsáveis por revisão/ausência/replay; a cerca não os substitui.

O adapter usa pool nativo `pg` próprio e limitado, com a mesma URL/credencial de autoridade. Isso evita que todos os leitores ocupem as conexões necessárias à revalidação SQL. Nenhuma credencial de autoridade é exposta ao adapter de identidade: a composição fornece apenas o port de coordenação. O pool retorna uma conexão somente depois de confirmar todos os unlocks na mesma sessão física; erro ou resultado ambíguo de aquisição, query ou unlock descarta o cliente com a API nativa pública. Aquisições usam `pg_try_advisory_lock[_shared]`, uma vez por chave confirmada, com espera entre tentativas e deadline original. Acesso público a `NodeHttpServerRequest.toServerResponse` pode observar o writer nos oráculos; não acessar campos privados.

Logout resolve a sessão real antes de adquirir seu exclusivo, chama o provider fora de transação semântica e conserva o exclusivo até a confirmação da ausência do ID exato. Falha de leitura prévia ou aquisição impede chamar logout sem coordenação; responde indisponível sem apagar cookie. O bug do provider descrito acima foi reproduzido e corrigido em `3595e36`, com contraprovas independentes `2f2b266`: 200 sozinho não confirma invalidação. A cerca ainda exige seus próprios oráculos de ordem concorrente.
