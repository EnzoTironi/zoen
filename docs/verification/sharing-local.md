...
A prova específica [SH-05 entre processos](../../tests/integration/d03-sharing/independent/http-lifecycle.review.md) também passou em dois cenários, com repetição independente por root em `dd4f781` (4,74 s). Locks reais demonstram dois backends aguardando antes de liberar as concessões. Mesmo opID produz respostas idênticas e um único efeito; opIDs distintos produzem 200/409 Stale e somente receipt vencedor. Replay nos dois servidores após revoke mantém o resultado histórico e a membership revogada. Essa execução acrescenta duas integrações às 166 do conjunto anterior; a conclusão da CI combinada está registrada abaixo.

### Primeira execução remota

A [CI de `40ab4db`](https://github.com/EnzoTironi/zoen/actions/runs/33992182271) passou em qualidade, 198 testes unitários, 168 integrações em 76 arquivos e sete cenários de componentes. A aceitação na imagem teve 11 cenários aprovados e uma falha: EX12 encontrou a sessão aberta depois de logout, navegação para `about:blank` e retorno. Frames e evidência estavam ausentes no screenshot. O agregador obrigatório reprovou corretamente essa execução. Logs e screenshots originais foram preservados; os resultados locais anteriores não substituem esse check remoto.

A [contraprova independente de logout](../../tests/integration/d03-sharing/independent/logout.review.md) reteve o POST antes do provider, observou as duas telas de login já limpas e navegou antes de liberar a chamada. O mesmo oráculo de retorno falhou, e o provider real ainda conservava a sessão. O contraste que observou sign-out 200 e get-session null antes da navegação passou. Não houve resposta de provider fabricada nem alteração do runtime. A correção `477cedd` acrescentou essa premissa aos cenários JSON/CSV, preservando as assertivas de login e remoção de conteúdo após a saída confirmada. A revisão independente concordou com o ajuste. Os 12 cenários combinados passaram novamente em 2,7 minutos no runtime `767f757`; log `acceptance-logout-20260905T2124.log`. A primeira repetição passou em todos os testes comportamentais, mas falhou na formatação do relatório; a conclusão posterior está registrada abaixo.

### CI combinada concluída

A [CI de `06535bd`](https://github.com/EnzoTironi/zoen/actions/runs/33993160944) passou nos 198 testes unitários, 168 integrações, sete cenários de componentes e 12 cenários na imagem. Qualidade falhou somente na formatação da tabela do relatório de logout. A correção preservou a fonte diagnóstica e sua evidência anterior.

A [CI de `aaa30db`](https://github.com/EnzoTironi/zoen/actions/runs/33993895643) concluiu com **todos os jobs aprovados**, incluindo o agregador obrigatório. Foram 198 testes unitários em 27 arquivos, 168 integrações em 76 arquivos, sete cenários de componentes e 12 cenários de aceitação na imagem `zoen-local:container-88f5e2d0f8f947cf` (3,2 minutos). Formatação, lint, TS7, build, import dos módulos compilados, verificação do plano e testes de tooling também passaram. O log integral foi preservado em `.local/sharing-ci-33993895643.log`.

EX20–EX23 ficam verificados para o perfil retido local e o escopo descrito neste documento. Isto não aprova erasure, restore, upgrade de Worlds antigos, cloud ou D03 integral.
